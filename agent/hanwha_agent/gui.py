...
class App:
    ...
    def _build_camera(self, nb):
        ...
        r += 1
        var = tk.BooleanVar(value=self.cfg.camera_clips)
        self.vars["camera_clips"] = var
        ttk.Checkbutton(page, text="Save a clip of the camera when an alarm happens",
                        variable=var).grid(row=r, column=0, columnspan=2, sticky="w", pady=(8, 0))
        r += 1
        row = tk.Frame(page, bg=BG)
        row.grid(row=r, column=0, columnspan=2, sticky="w", pady=(2, 0))
        for key, label, width in (("clip_pre_s", "seconds before", 5), ("clip_post_s", "and after", 5),
                                  ("camera_delay_s", "camera runs behind by (s)", 5)):
            v = tk.StringVar(value=str(getattr(self.cfg, key)))
            self.vars[key] = v
            ttk.Label(row, text=label, style="Form.TLabel").pack(side="left", padx=(0, 4))
            ttk.Entry(row, textvariable=v, width=width).pack(side="left", padx=(0, 14))
        r += 1
        ttk.Label(page, text="The picture is a few seconds behind real life, so the clip is shifted back by that much. "
                             "Check it against the machine's own clock and adjust if the alarm isn't in the middle.",
                  style="Hint.TLabel", wraplength=540, justify="left").grid(row=r, column=0, columnspan=2, sticky="w")
        r += 1
        btns = tk.Frame(page, bg=BG)
    ...
    def _refresh_camera(self):
        ...
        if not cam.enabled:
            ...
            if cam.error:
                text += f"\n{cam.error}"
        clips = self.clips
        if clips and clips.enabled:
            text += f"\nAlarm clips: {clips.status.lower()}"
            if clips.saved:
                text += f" · {clips.saved} saved"
            if clips.last_saved_at:
                text += f" · last {ago(clips.last_saved_at)}"
            pending = clips.info()["pending"]
            if pending:
                text += f" · {pending} waiting for the camera to catch up"
    ...
    def start_services(self):
        ...
        self.camera.start()
        self.clips = ClipRecorder(self.cfg)
        self.clips.start()
        self.collector.listeners.append(self.clips.on_snapshot)
