...
def main(argv=None):
    ...
    if args.headless:
        ...
        clips = ClipRecorder(cfg)
        clips.start()
        col.listeners.append(clips.on_snapshot)
        ...
        upd.start()
        signal.signal(signal.SIGINT, lambda *_: (cmd.stop(), cam.stop(), clips.stop(), col.stop(), up.stop(), sys.exit(0)))
        while True:
            time.sleep(1)
