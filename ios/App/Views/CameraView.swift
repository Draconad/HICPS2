...
final class CameraModel: ObservableObject {
    ...
    /// The monitor PC is what sends the video: with it off there's nothing live and the last still is stale.
    @Published var pcOffline = false
    private var stillEtag: String?

    func run(store: MachineStore) async {
        var tick = 0
        while !Task.isCancelled {
            let offline = store.status?.agentOnline == false
            if offline != pcOffline {
                pcOffline = offline
                if offline {         // drop the stale picture rather than pretend it's current
                    still = nil
                    stillEtag = nil
                    stillTime = nil
                }
            }
            if store.status?.camera?.available == true && !offline {
...
struct CameraPanel: View {
    ...
    private var hasPicture: Bool { !model.pcOffline && (model.still != nil || player.hasItem) }
    ...
    @ViewBuilder private var placeholder: some View {
        VStack(spacing: 10) {
            if model.pcOffline {
                ...
                Text("Monitor PC offline").font(.subheadline.weight(.semibold))
                Text("No camera while the PC on the machine is off.")
                    .font(.caption).foregroundStyle(.secondary).multilineTextAlignment(.center)
            } else if model.noCamera {
