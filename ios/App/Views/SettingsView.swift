...
struct SettingsView: View {
    ...
    @AppStorage(SettingsKey.notifyBattery) private var notifyBattery = true
        ...
        NavigationStack {
            Form {
                ...
                Section {
                    ...
                    Toggle("Backup battery due", isOn: $notifyBattery)
                ...
                Section {
                    NavigationLink {
                        ...
                        HStack {
                            Label("Backup battery", systemImage: "minus.plus.batteryblock")
                            Spacer()
                            if let m = store.status?.maintenance?.first(where: { $0.item == "battery" }) {
                                Text(m.headline)
                                    .font(.subheadline)
                                    .foregroundStyle(m.isOverdue ? MachineStateKind.alarm.color
                                                     : (m.state == "soon" ? Color.orange : Color.secondary))
                            }
                        }
                    }
                } header: {
                    ...
                    Text("When the memory backup batteries were last changed, and how often they need doing.")
                }
            ...
            .onChange(of: notifyBattery) { _ in push.registerAll() }
