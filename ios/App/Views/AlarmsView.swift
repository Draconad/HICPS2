        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 8) {
                    ...
                    if a.hasClip == true {
                        Label("Clip", systemImage: "play.rectangle.fill")
...
struct AlarmDetail: View {
    let a: AlarmEvent
    // built once: rebuilding it in body (the store publishes every poll) restarted the clip every few seconds
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                if a.hasClip == true {
                    Section {
                        ...
                        Text("The seconds either side of the alarm, saved by the PC app. The camera runs a little behind real life - that's already allowed for.")
                    }
                }
                Section("Details") {
                    row("Type", a.typeName ?? "—")
            ...
            .task {
                if a.hasClip == true, player == nil, let url = APIClient.current.clipURL(alarmID: a.id) {
