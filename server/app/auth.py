...
class Auth:
    ...
    # ------------------------------------------------------------------ user
    def _set_locked(self, username: str, password: str, must_change: bool, hashed=None):
        """hashed=(salt, pw_hash) when it was worked out beforehand - hashing takes a few hundred ms and
        shouldn't happen while the database lock is held."""
        salt, pw = hashed if hashed else (lambda s: (s, _hash(password, s)))(secrets.token_bytes(16))
        self.db.execute("""INSERT INTO auth_user(id,username,pw_hash,salt,iterations,must_change,updated)
                        ...
                        (username, pw, salt.hex(), ITERATIONS, int(must_change), time.time()))
    ...
    def change(self, token: str, current_password: str, new_username: str, new_password: str) -> str:
        ...
        hashed = (salt, _hash(new_password, salt))      # before taking the lock
        with self.lock:
            self._set_locked(new_username, new_password, must_change=False, hashed=hashed)
            self.db.execute("DELETE FROM sessions WHERE id<>?", (_token_id(token),))
