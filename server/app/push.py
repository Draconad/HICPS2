...
def row_prefs(row) -> dict:
    """A device's notification preferences, whatever ended up in the column."""
    try:
...
class PushService:
    ...
    def register(self, kind: str, token: str, activity_id: str | None = None, env: str | None = None,
        ...
        if kind not in ("alert", "la", "la_start") or not isinstance(token, str) or not token or len(token) > 400:
            raise ValueError("bad kind or token")
        # anything odd in the rest is dropped rather than stored: a bad row here would break every later tick
        activity_id = str(activity_id)[:200] if isinstance(activity_id, (str, int)) else None
        env = str(env)[:40] if isinstance(env, str) else None
        topic = str(topic)[:200] if isinstance(topic, str) else None
        prefs = {str(k): bool(v) for k, v in prefs.items()} if isinstance(prefs, dict) else None
        now = time.time()
        with self.lock:
    ...
    def _tick(self):
        ...
        def muted(row) -> bool:
            ...
            return bool(row_prefs(row).get("running_only")) and not in_window
        ...
        if jc.get("at") and jc["at"] != self.kv_get("push_job_complete_at") and now - jc["at"] < 600:
            for row in alert_rows:
                if row_prefs(row).get("complete", True):
        ...
        if s.get("bar_change") and since and now - since > limit and self.kv_get("push_bar_alert_for") != since:
            dur = int(now - since)
            for row in alert_rows:
                if row_prefs(row).get("barchange", True):
        ...
        # ---- maintenance: the machine's backup battery (and anything else on a schedule) -----------
        # One notification when it comes into its warning window, then a reminder a week at a time once it's
        # overdue. Not muted by "only while running" - it has nothing to do with what the machine is doing.
        due_items = [m for m in (s.get("maintenance") or []) if m.get("state") in ("soon", "overdue")]
        if due_items:
            with self.lock:
                maint_rows = self.db.execute("SELECT * FROM push_tokens WHERE kind='alert'").fetchall()
            for m in due_items:
                item, state = m.get("item") or "item", m["state"]
                marker = f"{state}:{round(m.get('due_at') or 0)}"
                sent_key, when_key = f"push_maint_{item}", f"push_maint_{item}_at"
                if self.kv_get(sent_key) == marker and (state == "soon"
                                                        or now - float(self.kv_get(when_key) or 0) < 7 * 86400):
                    continue
                days = int(round(abs(m.get("days_left") or 0)))
                plural = "" if days == 1 else "s"
                name = (m.get("name") or "Maintenance").lower()
                last = m.get("last_at")
                done = time.strftime("%d %b %Y", time.localtime(last)) if last else "never recorded"
                if state == "overdue":
                    title = f"🔋 {machine}: {name} overdue"
                    body = f"{days} day{plural} past due · last changed {done}"
                else:
                    title = f"🔋 {machine}: {name} due soon"
                    body = f"due in {days} day{plural} · last changed {done}"
                for row in maint_rows:
                    if row_prefs(row).get("battery", True):
                        self._send(row, {"aps": {"alert": {"title": title, "body": body},
                                                 "sound": "default", "thread-id": "maintenance"}},
                                   "alert", 5, collapse=f"maint{item}")
                self.kv_set(sent_key, marker)
                self.kv_set(when_key, now)
