...
Details:
- Each build only runs when its own folder changed (`ios/`, `agent/`, `server/`). GitHub works that out from the push, and the script checks again itself - comparing each folder against that build's own last successful run - and starts anything GitHub missed. (It misses them when this folder's git history was started fresh, e.g. a new copy of the project: the push then looks like unrelated history and nothing builds.) When a part genuinely wasn't rebuilt, the script downloads its last good build, so `build-out\` always has both files.
...
| `TZ` | | Your timezone, used for the "alarms today" and "bar changes today" counts, and for the day/month/year figures in the report. |
| `MAX_DOC_MB` | `25` | Biggest job PDF that can be attached to a program. They're kept in `docs/` in the data folder. |
| `MAX_CLIP_MB` | `60` | Biggest alarm camera clip the server will take. |
| `KEEP_CLIPS` / `KEEP_CLIP_DAYS` | `300` / `60` | How many alarm clips to keep, and for how long. They're in `clips/` in the data folder. |
...
### Program info and setup sheets
The **Program info** button at the top of the dashboard, or the **Programs** tab in the app, lists every program with a name, notes, setup details or bars on record. The loaded one is shown first. Open a program to:
- set its **name** (shown as O3110 - EMS301)
- set the **sub stick out** in mm for that job
- attach the **job PDF** - the drawing with the dimensions. Up to 25 MB; it opens in the app (pinch to zoom, share to print) and on the dashboard.
...
When a program with setup details is running, its stick out and a **Job PDF** button also show on the Program card, on the dashboard and in the app - so what's needed to set the job is on the main screen, not buried.

You can also add a program before it has run, to set its name or parts per bar in advance. Everything is stored on the server against the program number; the PDFs live in `docs/` in the server's data folder.

### Operations report
**Report** at the top of the dashboard, and the **Report** tab in the app: how many parts were made **by day, month or year**, how many bars were run, how long the machine spent running, stopped and in alarm, the average cycle time and parts per hour. Tap a day (or month, or year) to see which programs made them.

The figures are kept as a small day-by-day rollup as the snapshots come in, so the report is instant however far back you look. When the server is first updated, it fills in the history it already has from the recorded bars and cycle times; from then on it counts as it goes. Data is at `/api/report?period=day|month|year&limit=30`.

### Wall display
**Wall display** at the top of the dashboard is a full-screen page for a tablet or an old monitor by the machine: the state across the top in one colour you can read from the other end of the shop, the part count, cycle time, next bar change, the program with its stick out, when the job finishes, and any active alarms. Everything scales with the window, so it fits a phone, a tablet on its stand or a TV.

There's a **Full screen** button, a **Camera** button that adds the camera picture, and it asks the browser to keep the screen awake. Open `http://tower.local:8420/wall` directly (add `?key=…` if the API key is set) and leave it there - it looks after itself.

### Camera clip when an alarm happens
With **Save a clip of the camera when an alarm happens** ticked in the PC app's Camera settings, the PC keeps a rolling recording of the camera (short segments, copied straight from the camera with no re-encoding, and only the last few minutes kept). When an alarm comes in, the seconds either side are stitched together and sent to the server. A **Clip** button appears against that alarm in the dashboard's alarm history, and the app plays it inside the alarm.

The camera picture runs several seconds behind real life - about 12 on ours. So the clip window is **shifted back by that much**, and the clip isn't cut until the footage has actually arrived (the alarm's moment reaches the recording `camera runs behind by` seconds after it happened). Set **seconds before**, **and after** and **camera runs behind by (s)** in the PC app's Camera tab. If the alarm isn't in the middle of the clip, adjust that last figure: too low and the clip stops early, too high and it starts late.

Clips are kept on the server in `clips/` next to the database - the newest 300, and nothing older than 60 days (`KEEP_CLIPS`, `KEEP_CLIP_DAYS`).

### StandBy (iPhone on its side, charging)
With the phone charging on its side, iOS shows the Live Activity full screen. On iOS 18 and later the app switches to a bigger layout for it - state, part count, cycle time and what's left, in type you can read across the workshop. Nothing to set up: leave the phone on a charging stand and it's a machine display. There's no home screen widget; the Live Activity does the same job.
...
### Backup battery reminder
The FANUC control keeps its parameters and programs on memory backup batteries, and nothing warns you before they've already gone. So the date of the last change is kept on the server, with how often it should be done (a year by default), and everything counts down to it:

- **Dashboard:** nothing at all until it matters. Once it's inside the warning window (a month by default) a **large banner** appears across the top - orange for due soon, red and pulsing once it's overdue - with a *Changed today* button on it.
- **Settings** (the ⚙ button at the top of the dashboard, a new page): the date they were last changed, the interval (6 months to 3 years), how far ahead to warn, *Changed today*, and the dates of the previous changes.
- **Wall display:** a red strip along the bottom, readable from the machine.
- **App:** the same banner at the top of the dashboard, tappable, and the same settings at **Settings > Backup battery**.
- **Notification:** one when it comes into the warning window, then a reminder a week at a time while it stays overdue. Turn it off with **Backup battery due** in the app's notification settings. It isn't affected by "only while the machine is running" - it has nothing to do with what the machine is doing.

Recording it as changed starts the countdown again and keeps the old date in the history. The data is at `/api/maintenance`, and the same mechanism handles anything else on a schedule if more gets added later.
...
- **Monitor PC off:** the video and the last still are replaced by "Monitor PC offline", on the dashboard and in the app, rather than leaving an old picture on screen that looks live.
...
### Controls (iPhone app)
The controls live in a **Controls** card at the bottom of the app's main screen, folded away by default - tap it to open. (Before 1.10.0 they were a tab of their own; the tab is now Programs.)
...
3. If Push says **"no valid aps-environment entitlement"**, the App ID iLoader registered on your team doesn't have the push capability. Open [Identifiers](https://developer.apple.com/account/resources/identifiers/list), find the one matching the app, tick **Push Notifications** (and **Time Sensitive Notifications**), save, then delete the app from the phone and install it with iLoader again so it builds a fresh profile. That is usually all it takes.
4. Open HiCPS-2 → Settings → **Push (Apple)**:
...
## Testing the server
`server/tests_api.py` drives a running server through the things that go wrong in real life: rubbish snapshots, a missed poll in the middle of an alarm, store-and-forward arriving late, PDFs and clips, the battery dates, keys and logins. Point it at a **throwaway database** (it writes to it):

```bash
DB_PATH=/tmp/test.db PORT=8450 API_KEY=testkey123 python3 -m app.server &
python3 tests_api.py
```
...
`GET /api/status` · `GET /api/alarms?limit=100&before=<epoch>&active=1` · `GET /api/states?hours=24` · `POST /api/ingest` (agent) · `DELETE /api/alarms` (clears history) · `GET /api/health` · `GET /api/report?period=day|month|year&limit=30` · `GET /api/bars/day?date=2026-09-12` · `GET /api/programs` · `GET|PUT|DELETE /api/programs/doc?program=O3110` (the job PDF) · `GET /api/camera/clip?alarm=<id>` (the alarm's camera clip) · `GET /api/camera/clips` · `GET /api/maintenance` · `POST /api/maintenance {item, changed|last_at, every_months}` (the backup battery). When `API_KEY` is set, send the key as the `X-API-Key` header or as `?key=`. A logged-in dashboard session also works.
