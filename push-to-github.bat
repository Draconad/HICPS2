...
REM ---- which builds are needed --------------------------------------------
REM Each build normally starts itself, because GitHub sees which folder changed
REM in the push. That check is GitHub's, and it comes up empty if this folder's
REM history was started fresh (a new copy of the project, say) - the push then
REM looks like unrelated history and nothing builds. So the same question is
REM asked here as well, against each build's own last successful run, and
REM anything GitHub missed is started by name.
set "HEADSHA="
for /f "tokens=*" %%A in ('git rev-parse HEAD 2^>nul') do set "HEADSHA=%%A"
echo.
echo Checking which builds are needed...
REM a moment for the builds the push started itself to show up
timeout /t 8 /nobreak >nul 2>nul

REM ---- iPhone app: only built when asked (macOS build minutes count 10x) ----
call :needs_build ios ios.yml
if "%NEEDS%"=="0" goto after_ios
echo.
...
REM ---- Windows app and server image: started automatically when needed -----
call :needs_build agent agent.yml
if "%NEEDS%"=="0" goto after_agent
echo   Windows app: starting the build (GitHub didn't).
gh workflow run agent.yml --repo %REPO% --ref main -f include_ffmpeg=false
if errorlevel 1 echo   Couldn't start it - Actions tab ^> Windows agent ^> Run workflow.
:after_agent

call :needs_build server server.yml
if "%NEEDS%"=="0" goto after_server
echo   Server image: starting the build (GitHub didn't).
gh workflow run server.yml --repo %REPO% --ref main
if errorlevel 1 echo   Couldn't start it - Actions tab ^> Server Docker image ^> Run workflow.
:after_server
...
REM ---- does this part need building? ---------------------------------------
REM   %1 = folder, %2 = its workflow file.  Sets NEEDS to 1 or 0.
REM   a build of this exact commit already exists  -> 0 (the push started it)
REM   nothing ever built successfully              -> 1
REM   the folder changed since that build          -> 1
REM   (also 1 when the commit it was built from isn't in this folder's history
REM    any more, which is the case this check exists for)
:needs_build
set "NEEDS=1"
if not defined HEADSHA goto :eof
for /f "tokens=*" %%A in ('gh run list --repo %REPO% --workflow %~2 --limit 15 --json headSha --jq ".[].headSha" 2^>nul') do if /i "%%A"=="%HEADSHA%" set "NEEDS=0"
if "%NEEDS%"=="0" goto :eof
set "LASTSHA="
for /f "tokens=*" %%A in ('gh run list --repo %REPO% --workflow %~2 --status success --limit 1 --json headSha --jq ".[0].headSha" 2^>nul') do set "LASTSHA=%%A"
if /i "%LASTSHA%"=="null" set "LASTSHA="
if not defined LASTSHA goto :eof
git diff --quiet %LASTSHA% HEAD -- %~1 >nul 2>nul
if not errorlevel 1 set "NEEDS=0"
goto :eof
